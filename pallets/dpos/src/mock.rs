...
parameter_types! {
	...
	pub static ValidatorCommission : u32 = 15; // 0.3
	pub static DelegatorCommission : u32 = 3; // 0.1
}
...
pub struct TestExtBuilder {
	genesis_candidates: CandidateSet<Test>,
	balance_rate: u32,
...
impl Default for TestExtBuilder {
	fn default() -> Self {
		Self {
			genesis_candidates: DEFAULT_ACTIVE_SET.to_vec(),
...
impl TestExtBuilder {
	...
	pub fn genesis_candidates(&mut self, candidates: CandidateSet<Test>) -> &mut Self {
		self.genesis_candidates = candidates;
		self
	}
	...
	fn with_storage(&self) -> sp_io::TestExternalities {
		...
		let _ = pallet_dpos::GenesisConfig::<Test> {
			genesis_candidates: self.genesis_candidates.clone(),
